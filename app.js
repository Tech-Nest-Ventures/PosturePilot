class PosturePilot {
    ...
    async initializePoseDetection() {
        ...
        if (typeof Pose === 'undefined') {
            throw new Error('MediaPipe Pose library is not loaded. Please ensure all MediaPipe scripts are loaded correctly.');
        }
        ...
        try {
            ...
            // Paths are relative to the HTML file location (src/index.html)
            this.pose = new Pose({
                locateFile: (file) => {
...
// Add error handling to prevent crashes in MAS builds
document.addEventListener('DOMContentLoaded', () => {
    try {
        // Check if MediaPipe is available before initializing
        if (typeof Pose === 'undefined') {
            console.error('MediaPipe Pose is not available. Check if scripts loaded correctly.');
            // Show error message to user
            const errorDiv = document.createElement('div');
            errorDiv.style.cssText = 'position: fixed; top: 0; left: 0; right: 0; background: #ef4444; color: white; padding: 20px; text-align: center; z-index: 10000;';
            errorDiv.textContent = 'Error: MediaPipe failed to load. Please restart the app.';
        ...
        console.error('Failed to initialize PosturePilot:', error);
        // Show error message to user instead of crashing
        const errorDiv = document.createElement('div');
        errorDiv.style.cssText = 'position: fixed; top: 0; left: 0; right: 0; background: #ef4444; color: white; padding: 20px; text-align: center; z-index: 10000;';
        errorDiv.textContent = `Error initializing app: ${error.message}. Please restart the app.`;
        document.body.appendChild(errorDiv);
    }
});